...
const NAMES = ['Arjun', 'Aisha', 'Rohan', 'Meera', 'Kabir', 'Nisha', 'Siddharth', 'Ananya'];
...
const MAX_DISCOVERY_RADIUS = 3000;

function randomBetween(min, max) {
  return Math.floor(Math.random() * (max - min + 1)) + min;
}

function getRadarPosition(distance, index, total) {
  const maxRadarRadius = 45;
  const minRadarRadius = 12;

  const normalized = distance / MAX_DISCOVERY_RADIUS;

  const radarRadius = minRadarRadius + normalized * (maxRadarRadius - minRadarRadius);
        ...
        <div className="orbit-left">
          ...
          <div className="radius-box">
            ...
            <div className="radius-scale">
              ...
              <span className="radius-scale-text">
                {MAX_DISCOVERY_RADIUS >= 1000
                  ? `${MAX_DISCOVERY_RADIUS / 1000}km`
                  : `${MAX_DISCOVERY_RADIUS}m`}
              </span>
            </div>
            ...
            {people.map((person) => {
              ...
              return (
                ...
                >
                  ...
                  {visible && <div className="distance">{person.distance}m</div>}
                </div>
              );
            })}
